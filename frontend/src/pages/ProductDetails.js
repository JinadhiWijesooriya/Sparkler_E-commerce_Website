...
import { getProductById, getProducts } from "../api/shopApi";
import { useCart } from "../context/useCart";
/* -------------------------------------------------------------------------- */
/*                                 COMPONENTS                                 */
/* -------------------------------------------------------------------------- */
const SpecItem = ({ icon: Icon, label, value }) => (_jsxs("div", { className: "flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm group hover:border-[#C9A24D]/50 transition-all", children: [_jsx("div", { className: "w-10 h-10 rounded-xl bg-[#C9A24D]/10 flex items-center justify-center text-[#C9A24D] group-hover:scale-110 transition-transform", children: _jsx(Icon, { size: 20 }) }), _jsxs("div", { children: [_jsx("p", { className: "text-white/40 text-xs uppercase tracking-widest font-bold", children: label }), _jsx("p", { className: "text-white font-medium", children: typeof value === 'boolean' ? (value ? "Certified" : "Verify Required") : (value || "N/A") })] })] }));
/* ---------------- MAP API PRODUCT TO FRONTEND TYPE ---------------- */
const mapAPIProductToCard = (p) => ({
    id: p.id,
    name: p.name,
    price: p.price,
    description: p.description ?? "Handcrafted luxury jewelry.",
    images: p.images && p.images.length > 0 ? p.images.map((img) => img.image) : ["/placeholder.jpg"],
    product_type: "shop",
});
export default function ProductDetails() {
        ...
        const loadData = async () => {
            try {
                ...
                if (productData) {
                    ...
                    setSelectedImage(productData.images[0]?.image || "/placeholder.jpg");
                    const allProducts = await getProducts({ availability: true });
    ...
    const handleAddToCart = () => {
        if (!product)
            return;
        addToCart(product.id, "shop", quantity);
    };
    const handleInquiry = () => {
        if (!product)
            return;
        const whatsappNumber = "94705696254";
        const message = `Hello Sparkler! I am inquiring about this exquisite jewelry piece:
- Name: ${product.name}
- Gem: ${product.gem}
- Metal: ${product.metal}
...
- Ref ID: #${product.id}`;
    ...
    if (loading) {
        return (_jsxs("div", { className: "min-h-screen bg-black flex flex-col items-center justify-center", children: [_jsx("div", { className: "w-16 h-16 border-4 border-[#C9A24D]/20 border-t-[#C9A24D] rounded-full animate-spin mb-4" }), _jsx("p", { className: "text-[#C9A24D] font-bold tracking-widest uppercase text-sm animate-pulse", children: "Illuminating Masterpiece..." })] }));
    }
    if (!product) {
        return (_jsxs("div", { className: "min-h-screen bg-black flex flex-col items-center justify-center text-center px-4", children: [_jsx("h2", { className: "text-4xl font-bold text-white mb-4", children: "Piece Not Found" }), _jsxs("button", { onClick: () => navigate("/shop"), className: "text-[#C9A24D] flex items-center gap-2 hover:underline font-bold uppercase tracking-widest text-sm", children: [_jsx(ChevronLeft, { size: 20 }), " Revisit Collection"] })] }));
    }
