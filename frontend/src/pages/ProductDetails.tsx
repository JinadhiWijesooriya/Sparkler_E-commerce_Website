...
import type { Product as APIProduct } from "../api/shopApi";
...
const SpecItem = ({ icon: Icon, label, value }: { icon: LucideIcon, label: string, value: string | number | null | boolean }) => (
  <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm group hover:border-[#C9A24D]/50 transition-all">
    ...
    <div>
      ...
      <p className="text-white font-medium">{typeof value === 'boolean' ? (value ? "Certified" : "Verify Required") : (value || "N/A")}</p>
    </div>
  </div>
);

/* ---------------- MAP API PRODUCT TO FRONTEND TYPE ---------------- */
const mapAPIProductToCard = (p: APIProduct): ProductCardType => ({
  id: p.id,
  name: p.name,
  price: p.price,
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center">
        ...
        <p className="text-[#C9A24D] font-bold tracking-widest uppercase text-sm animate-pulse">Illuminating Masterpiece...</p>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-center px-4">
        <h2 className="text-4xl font-bold text-white mb-4">Piece Not Found</h2>
        <button
          onClick={() => navigate("/shop")}
          className="text-[#C9A24D] flex items-center gap-2 hover:underline font-bold uppercase tracking-widest text-sm"
        >
          <ChevronLeft size={20} /> Revisit Collection
        </button>
      </div>
    );
  }
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        ...
        <div className="space-y-6">
          <motion.div
            layoutId={`shop-image-${product.id}`}
        ...
        <div className="space-y-10 py-4">
          <div className="space-y-4">
            ...
            >
              <Sparkles size={16} />
              <span className="text-xs font-bold uppercase tracking-[0.3em]">Fine Jewelry</span>
            </motion.div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold uppercase tracking-wider">
                {product.availability ? "Available" : "Bespoke Only"}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <SpecItem icon={Hexagon} label="Carat" value={`${product.carat}ct`} />
            <SpecItem icon={ShieldCheck} label="Status" value={product.certification} />
          </div>
